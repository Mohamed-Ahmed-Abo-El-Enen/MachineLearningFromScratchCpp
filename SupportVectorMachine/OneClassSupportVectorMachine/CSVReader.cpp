...
vector<Sample> Sample::ConvertCSVSamples(vector<CSVSample> csvArr, vector<int> categorical_cols, int yIndex, map<string, int>& class_map)
{
	...
	for (size_t i = 0; i < csvArr.size(); i++)
	{
		...
		for (int c = 0; c < csvArr[0].values.size(); c++)
		{
			if (c == yIndex)
				continue;
...
void Sample::ConvertCSVSamplesXY(vector<CSVSample> csvArr, vector<int>  categorical_cols, int yIndex, map<string, int>& class_map, vector<vector<double>>& X, vector<int>& y)
{
	vector< map<string, double>> feature_map(categorical_cols.size(), map<string, double>());

	for (size_t i = 0; i < csvArr.size(); i++)
	{
		vector<double> sampleFeatures;
		int cat_col_index = 0;
		...
		for (int c = 0; c < csvArr[0].values.size(); c++)
		{
			...
			if (find(categorical_cols.begin(), categorical_cols.end(), c) == categorical_cols.end() || categorical_cols.size() <= 0)
				sampleFeatures.push_back(stod(csvArr[i].values[c]));
			else
			{
				feature_map[cat_col_index].insert({ csvArr[i].values[c], feature_map[cat_col_index].size() });
				sampleFeatures.push_back(feature_map[cat_col_index].at(csvArr[i].values[c]));
				cat_col_index++;
			}
		}
		X.push_back(sampleFeatures);
	}
	...
	for (int i = 0; i < csvArr.size(); i++)
		y.push_back(class_map.at(csvArr[i].values[yIndex]));
}
